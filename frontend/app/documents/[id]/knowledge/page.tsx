...
function EquationNode({ data }: { data: CardNodeData }) {
  const html = useMemo(() => {
    try {
      return katex.renderToString(data.label, { throwOnError: false });
    } catch {
      return data.label;
    }
  }, [data.label]);
...
async function layoutWithElk(
  ...
  filters: Filters,
): Promise<{ nodes: Node<CardNodeData>[]; edges: Edge[] }> {
...
function KnowledgeGraphInner() {
  ...
  const filteredEdges: Edge[] = edges.map((e) => {
  ...
  return (
    <div className="flex h-screen flex-col bg-[#fafafa]">
      <header className="flex shrink-0 items-center border-b border-slate-200 px-6 py-4 bg-white/80 backdrop-blur">
        ...
        <h1 className="ml-4 text-lg font-semibold text-slate-900">Knowledge graph</h1>
      </header>
